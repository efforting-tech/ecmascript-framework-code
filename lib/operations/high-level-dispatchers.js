...
__context__.set(${JSON.stringify(path)}, __d);
/*__ft.set(${JSON.stringify(path)}, function ${name}(${joined_signature_arguments}) {
...
export function create_dispatchers_from_raster_table(context, raster) {
	...
	const function_tree = create_dispatch_tree_from_descriptors(context, pending_dispatchers);
