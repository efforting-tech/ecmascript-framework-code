...
/*export class Tabulators {
	...
	static get_level_from_span(span) {
	...
	static to_component_spans(text, sub_span=undefined) {
		if (sub_span) {
			...
			log.Debug('Tabulator.to_component_spans', text, this.indexed_component_pattern, text.match(this.indexed_component_pattern));
			return text.match(this.indexed_component_pattern).indices.slice(1);
		}
	}
