...
export class Line {
	...
	copy_trimmed_also_old(line_left=0, line_right=0, title_left=0, title_right=0) {
		const { owner, head_span, title_span, tail_span, line_index, column_index } = this;
		const [[h1, h2], [ti1, ti2], [ta1, ta2]] = [head_span, title_span, tail_span];
		...
		const nhl = Math.max(0, hl - line_left);
		const lrem1 = line_left - (hl - nhl);

		const ntil = Math.max(0, til - lrem1);
		const lrem2 = lrem1 - (til - ntil);

		const ntal = Math.max(0, tal - lrem2);
		const lrem3 = lrem2 - (tal - ntal);



		console.log(hl, nhl, lrem1);
		console.log(til, ntil, lrem2);
		console.log(tal, ntal, lrem3);


		console.log((hl + til + tal) - (nhl + ntil + ntal));

		const p1 = h1 + line_left;
		const p2 = p1 + hl
	...
	copy_trimmed_old(left=0, head_title=0, title_tail=0, right=0) {
		//TODO - check bounds
		//We must make sure that if we cut too much of left or right it starts eating of head or title. But for now this is not implemented and caller must make sure to be aware of this limitation
		...
		const p1 = h1 + left;
		const p2 = p1 + hl - left + head_title;
		const p3 = p2 + til - head_title + title_tail;
		const p4 = p3 + tal - title_tail - right;

		const new_head_span = [p1, p2];
		const new_title_span = [p2, p3];
		const new_tail_span = [p3, p4];
		const new_column_index = column_index + left;

		console.log(head_span, title_span, tail_span);
		console.log(new_head_span, new_title_span, new_tail_span);

		return new this.constructor(owner, new_head_span, new_title_span, new_tail_span, line_index, new_column_index);
