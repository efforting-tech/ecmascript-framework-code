...
export function escape_regex(r) {
...
export function create_system(AST={}, records_tree=new Dotted_Tree_Directory(), dispatch_tree=new Dotted_Tree_Directory()) {
	Object.assign(AST, { RE, escape_regex, process_placeholder });
	...
	create_records_from_raster_table(AST, records_tree, `
		name			group				members
		----			-----				-------
		Text			default				value
		Whitespace		〃					〃
	...
	create_dispatchers_from_raster_table(AST, dispatch_tree, `
		name							group				constructor				expression
		----							-----				-----------				----------
		to_regex(context, §item)		default				Array					RE.update_flag(RE.concat('^', ...item.map(sub_item => __flat.to_regex(context, sub_item)), '$'), 'i', true)
	...
	add_rule(tp_main_processor, 'record table[:]', (resolver, node, match, { data_path }) => {
		create_records_from_raster_table(context, records_tree, node.body.to_text());
	});

	add_rule(tp_main_processor, 'constructor based dispatch table[:]', (resolver, node, match, { data_path }) => {
		create_dispatchers_from_raster_table(context, dispatch_tree, node.body.to_text());
	});
	...
	add_rule(tp_tokenizer_processor, 'capture «comma separated identifiers»[:] «pattern»', (resolver, node, match, { identifiers, pattern }) => {
		...
		context.target_tokenizer.add_rule(new R.Resolution_Rule(new C.Regex_Condition( eval(pattern) ), (...captures) => {
...
export function get_by_path(context, path) {
	let ptr = context;
...
//TODO - move to data/object
export function set_by_path(context, path, value) {
	let ptr = context;
