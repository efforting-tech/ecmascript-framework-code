...
class Generic_Tokenization_State {
	constructor(system, text, start_position=0, position=start_position, tokenizer=system.ingress, pending_position=null, value=[]) {
		Object.assign(this, { system, text, start_position, position, tokenizer, pending_position, value })
	...
	*[Symbol.iterator]() {
	...
	perform_match_action(system, match, action) {
		...
		if (action instanceof AST.Emit_Token) {
			const { token, pending_actions } = action;
			...
			if (action.pending_actions.length > 0) {
				for (const sub_action of pending_actions) {
					this.perform_match_action(system, match, sub_action)
				}
			}
