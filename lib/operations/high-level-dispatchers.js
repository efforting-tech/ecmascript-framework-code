...
//NOTE - these might be deprecated later and instead simplified and put in the tree language part of the project

export function create_record_descriptors_from_table(table) {
	const result = [];
	for (const { name, group, members } of table) {
...
export function create_record_tree_from_descriptors(context, pending_records) {
// Create records
...
context.set(${JSON.stringify(path)}, __t);
		`;
...
export function create_dispatch_tree_from_descriptors(context, pending_dispatchers) {
	...
	for (const { path, group, name, main_argument, arg_entries, signature_arguments, rules } of Object.values(pending_collated_dispatchers)) {
		...
		let code = `
const __d = new T.Constructor_Based_Mapping_Processor(${JSON.stringify(path)});
context.set(${JSON.stringify(path + '.processor')}, __d);

const pending_function = function ${name}(${joined_signature_arguments}) {
	return __d.process({${joined_arg_entries}}, ${main_argument})
}
context.set(${JSON.stringify(path)}, pending_function);
${path} = pending_function;	//HACK: Context is not updated between context.set and here so we do this which might fail to populate path components
		`.trimRight();
		...
		for (const { expression, constructor } of rules) {
			const full_constructor_path = constructor.startsWith('.') ? `${group}${constructor}` : constructor;	//TODO - document this feature
			pre_check += `\nif (!${full_constructor_path}) { throw new Error('Missing reference to ${full_constructor_path} in dispatcher in ${context.get_dir().path ?? "(anonymous)"}')};`
		...
		code = pre_check + code + post_check;
		context.execute(code);
...
export function create_dispatchers_from_raster_table(context, raster) {
	...
	create_dispatch_tree_from_descriptors(context, pending_dispatchers);
}


export function create_records_from_raster_table(context, raster) {

	const records = load_ditto_raster(raster, 'name');
	const pending_records = create_record_descriptors_from_table(records);
	create_record_tree_from_descriptors(context, pending_records);
}
