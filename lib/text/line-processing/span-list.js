...
import { clamped_adjustment, get_title_start_point, get_title_end_point, DEFAULT_INDENTION_MODE, DEFAULT_INDENTION_WIDTH, INDENTION_MODE } from './common.js';
...
export class Line {
	...
	static from_span(owner, span_left, span_right, line_index=undefined, column_index=undefined) {
		...
		result.title_span_left = get_title_start_point(result.full_line);	//TODO - respect indention settings here
		result.title_span_right = get_title_end_point(result.full_line);

		return result;
	}

	indented_copy(adjustment=0) {
		return new Line_With_Adjusted_Indent(this, adjustment);
	}
