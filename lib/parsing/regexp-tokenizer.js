...
//TODO - I have not decided exactly the roles of find_matches and feed, I might rename or fix up this a bit - it is currently brittle
...
export class Advanced_Regex_Tokenizer {
	...
	[inspect.custom](depth, options, inspect) {
		const { name, rules } = this;
		return `${this.constructor.name}(${inspect(name, options)} with ${inspect(rules.length, options)} rules)`;
	}

	add_rule(...rules_to_add) {
	...
	*find_matches(text, position=0) {
		while (true) {
			const pending = this.find_next_match(text, position);
			if (pending) {
				yield pending;
			}
	...
	find_next_match(text, position=0) {
		...
		if (best_match) {
			...
			if (head.length) {
				return this._handle_default_match(head, position, best_match.match.index);
			}
		}
...
export function tokenize_block(tokenizer, block) {
	const result = [];
	for (const match of	tokenizer.find_matches(block)) {
