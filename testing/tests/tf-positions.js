...
//TODO - this should be implemented as function in some text processing class
...
for (const [experiment_title, format, text] of experiments) {
	...
	for (const [level, head_span, title_span, tail_span] of format.iter_level_and_component_spans(text)) {
		const title_column_index = pending_column_index + format.indention_mode.level_to_visual_column(level);
