...
export function create_block_rule(tag, handler, include_type=REQUIREMENT_STATE.OPTIONAL, include_name=REQUIREMENT_STATE.OPTIONAL, include_settings=REQUIREMENT_STATE.OPTIONAL, ignore_case=true, name_pattern=/\w+/) {
	const { pattern, groups } = create_block_rule_pattern(tag, include_type, include_name, include_settings, ignore_case, name_pattern);
...
//TODO - we have so many settings here - we should make it into an object (maybe even a configuration schema object)
export function create_block_rule_pattern(tag, include_type=REQUIREMENT_STATE.OPTIONAL, include_name=REQUIREMENT_STATE.OPTIONAL, include_settings=REQUIREMENT_STATE.OPTIONAL, ignore_case=true, name_pattern=/\w+/) {
	const pieces = [/^/];
