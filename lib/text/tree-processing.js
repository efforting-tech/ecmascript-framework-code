...
export class Node {
	...
	copy_with_new_owner(new_owner) {
		const new_copy = new this.constructor(new_owner, this.settings, this.parent);
		return new_copy;
	}

	copy_body_under_new_title(new_title, column_adjustment=0) {
		const new_owner = this.owner.copy_with_new_title(new_title, column_adjustment);
		return this.copy_with_new_owner(new_owner);
	}


	static from_string(string, first_line_index=0, first_column_index=0, settings=DEFAULT_TREE_SETTINGS) {
		return new this(Span_List.View.from_string(string, first_line_index, first_column_index, settings.text_format), settings);
	}

	static from_lines(lines, source=undefined, first_line_index=0, first_column_index=0, settings=DEFAULT_TREE_SETTINGS) {
		return new this(String_List.View.from_lines(source, lines, first_line_index, first_column_index, settings.text_format), settings);
	}

	static from_children(children, first_line_index=0, first_column_index=0, settings=DEFAULT_TREE_SETTINGS) {
		...
		for (const node of children) {
			if (node instanceof this) {
				lines.push(...node.to_lines());
			} else if (Array.isArray(node)) {
				lines.push(...node);	//TODO - maybe we should not assume it is raw lines
			}
		}

		return new this(String_List.View.from_lines(children, lines, first_line_index, first_column_index, settings.text_format), settings);
	}
	...
	reindented_copy(adjustment=0) {
		const copied_owner = this.owner.reindented_copy(adjustment);
		return new this.constructor(copied_owner, this.settings);
	}
	...
	to_lines() {
		return [...this.owner.map(line => line.head_and_title)];
	}
...
*/
	...
	copy(title=undefined, body=undefined) {
		...
		if (body !== undefined) {
			throw new Error('Not properly handled yet');
			const result = this.constructor.from_children([this.constructor.from_string(title ?? ''),  body]);
			console.log(result)
			return result;
		...
		if (body !== undefined) {
			...
			if (Array.isArray(body)) {
				...
				for (const node of body) {
					new_lines.push(...node.lines.map(line => line.copy()));
				}
				result.owner.lines.splice(1, Infinity, new_lines);

				console.log(result.owner.lines)
