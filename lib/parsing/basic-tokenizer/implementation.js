...
export class Token_Match {
	...
	get text() {
		return this.input.slice(this.index, this.end_index);
	}
...
class Generic_Tokenization_State {
	...
	*[Symbol.iterator]() {
	...
	perform_match_action(system, match, action) {
		...
		} else if (action instanceof AST.Call_Function_Emit_Value) {

			const { function_reference, token_processor } = action;
			const tp = system.resolve_token_processor(action.token_processor);
			assert_defined(tp);
			this.value.push(function_reference({system, match, token_processor: tp }));
			this.pending_position = match.pending_index;
	...
	add_regexp_rule(pattern, action) {
		...
		if (Array.isArray(pattern)) {
			...
			for (const sub_pattern of pattern) {
				const regexp = system.regexp_resolver(sub_pattern);
