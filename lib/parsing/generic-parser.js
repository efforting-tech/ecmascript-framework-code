...
//NOTE: We use a tokenizer here but we will execute the actions of that tokenizer differently. This means that a tokenizer must be defined with the parser in mind for this to work.
export class Parser {

	constructor(source, tokenizer, state={}) {
		this.token_generator = new Switchable_Iterator();

		const pending_state = {
			pending_position: 0,
			pending_value: [],
			sub_tokenizer_handlers: [],
			named_tokenizers: {},
			...state,
		};

		init_stack_channel(this, 'stack', { tokenizer, ...pending_state })
		this.switch_to(tokenizer, this.pending_position, source);
	}
	...
	switch_to(tokenizer, pending_position=undefined, source=undefined) {
		...
		const resolved_by_name = this.named_tokenizers[tokenizer];
		if (resolved_by_name) {
			tokenizer = resolved_by_name;
		}

		if (!tokenizer?.feed) {
			if (typeof tokenizer === 'string') {
				throw new Error(`No such named tokenizer: ${tokenizer}`);
			} else {
				throw new Error(`Protocol mismatch for tokenizer: ${tokenizer}`);
			}
