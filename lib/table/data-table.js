...
export class Multi_View_Table {
	...
	update_all_cells(callback) {
		for (let row_index = 0; row_index < this.rows.length; row_index++) {
			const row = this.rows[row_index];
			for (let col_index = 0; col_index < row.length; col_index++) {
				const value = row[col_index];
				row[col_index] = callback(this, row_index, col_index, value);
			}
		}
	}
...
export function table_from_raster(raster, tab_width=4, null_padding=true) {
	if (raster.match(/\t/g)) {
	...
	const column_indices = [...column_matches.map(cm => cm.indices[1][0])]
	const columns = [...column_matches.map(cm => cm[1])]
	const result = [];
	...
	for (const row of m[3].matchAll(row_pattern)) {
		...
		for (let ci=0; ci<column_indices.length; ci++) {
			const cell = row[1].slice(column_indices[ci], column_indices[ci+1]);
			if (null_padding) {
...
export function load_ditto_raster(raster, multiline_key=null, ditto_marker=/〃/g) {
	const table = Multi_View_Table.from_raster(raster)

	table.visit_all_cells((table, row, col, value) => {
		if (value?.match(/〃/g)) {
			const above = table.require_cell(row-1, col);
			value = value.replace(ditto_marker, table.rows[row - 1][col])
		}
		table.write_cell(row, col, value.trim());
	});

	function join_rows(rows) {
		const result = [];
		if (rows.length > 0) {
			for (let col=0; col<table.columns.length; col++) {
				result.push(rows.map(r => r[col]).join('\n').trim());
			}
		}
		return result;
	}
	...
	if (multiline_key) {
		...
		for (const row of table) {
			if (row[multiline_key].length > 0) {
				if (pending.length > 0) {
					collated_rows.push(join_rows(pending));
				}
				pending.length = 0;
			}
			pending.push([...row])
