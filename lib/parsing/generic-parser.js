import { Switchable_Iterator } from './switchable-iterator.js';
import { init_stack_channel } from './stack-channel.js';
...
export class Parser {
	...
	switch_to(tokenizer, pending_position=undefined, source=undefined) {
		...
		this.tokenizer.state.parser = this;
	...
	leave_sub_tokenizer() {
		...
		if (this.sub_tokenizer_handlers.length) {
			...
			this.push_token(handler(this, frame.pending_value));
		} else {
